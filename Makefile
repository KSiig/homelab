...
sops-setup:
	ssh -p 2222 $(PI_USER)@$(PI_HOST) 'sudo cat /etc/sops/age/keys.txt' | head -n 1
	@echo ""
	@echo "Copy the public key above into .sops.yaml under 'age:'"
	@echo "Then run: make flux-sops-secret"

flux-sops-secret:
	ssh -p 2222 $(PI_USER)@$(PI_HOST) 'sudo k3s kubectl create secret generic sops-age \
...
kubeconfig:
	...
	@scp -P 2222 $(PI_USER)@$(PI_HOST):/etc/rancher/k3s/k3s.yaml $(KUBECONFIG_PI)
